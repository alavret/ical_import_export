...
- [Обзор](#обзор)
- [Параметры](#параметры)
- [Важная информация при создании событий в Яндкс 360](#важно-что-нужно-знать-о-создании-событий-в-календаре-яндекс-360)
...
| `USER_MAPPING_FILE` | Путь к файлу маппинга пользователей (соответствие алиасов внешней системы и Яндекс 360) | Нет (по умолчанию `user_mapping.txt`) | `user_mapping.txt` |
...
# Путь к файлу маппинга пользователей (внешняя система → Яндекс 360)
USER_MAPPING_FILE = user_mapping.txt
```

## ВАЖНО! Что нужно знать о создании событий в Календаре Яндекс 360

### Конфликт уникальных идентификаторов событий

У каждого события, которое находится в Календаре обслуживаемого CalDav сервером, есть уникальное поле UID. Все события внутри одного CalDav в сервера имеют уникальный UID. Это правило, в том числе, относится к событиям внутри Календаря Яндекс 360. Когда вы импортируете события из внешнего CalDav сервера, либо другой системы управления событиями, например Exchange сервер, вы сохраняете события в файле формата iCal и внутри этого файла есть поле UID, которое содержит уникальный идентификатор событий в исходной системе. Если вы будете импортировать эти события в календарь Яндекс 360, то они будут созданы внутри Яндекс 360 с их исходными уникальными идентификаторами.
Однако, если вы уже однажды импортировали такое событие, затем удалили его в календаре Яндекс 360, а потом снова попытались импортировать тоже самое событие, вы получите ошибку создания нового события. Это происходит потому, что календарь Яндекс 360 сохранил исходный UID события своей базе данных, и больше создать событие с таким UID нет возможности (Яндекс Rалендарь не удаляет UID событий из своей базы данных даже после удаления самого события).
В связи с этим, при повторном импорте событий, которые ранее уже были импортированы в календарь Яндекс 360, необходимо использовать опцию генерации нового UD для каждого импортируемого события. Также есть опция замены существующего события с таким же UID.

### Cоздание события, организатором которого является не владелец почтового ящика

При импорте событий из других систем очень часто возникает ситуация, когда организатор встречи является не владелец календаря, куда происходит миграция новых событий. В этом случае, возможны два варианта:
 - email пользователя, который указан как организатор события, зарегистрирован в каталоге Яндекс 360 в вашей, либо в какой-либо другой организации. То есть, такой пользователь существует каталоге Яндекс 360. В этом случае, если организатор не является владельцем Календаря, куда происходит импорт данного события, события не будет создано. Такое событие может быть создано только в Календаре Яндекс 360, владельцем которого является организатор события.
 - email пользователя, который указан как организатор события, не зарегистрирован в каталоге Яндекс 360. Это может быть даже пользователь с электронным адресом в домене вашей организации, но который не был мигрирован в сервис Яндекс 360. Но, обычно, это пользователь во внешней почтовой системе. В таком случае, событие в Календаре пользователя  Яндекс 360, куда происходит миграция, может быть создано, но такое событие владелец Календаря не сможет отредактировать. Если необходимо иметь возможность редактирования создаваемого события, необходимо воспользоваться опцией замены организатора собрания на владельца Календаря, где происходит создание, при импорте событий.

### Отправка приглашений для импортируемых событий

При процедуре импорта, для вновь создаваемых событий, не происходит отправка приглашений участникам события. Это достигается с помощью добавления специального заголовка в запрос при создании события.
Однако при модификации существующих событий происходит отправка уведмлений участникам событий.
Также, при удалении событий, происходит отправка уведомлений всем участникам. Для того, чтобы исключить получение участниками уведомлений об удалении событий, рекомендуется воспользоваться транспортным правилом в Яндекс 360, которое блокирует подобные сообщения. Для этого есть специальный пункт меню.
...
#### Файл маппинга пользователей (`user_mapping.txt`)

Текстовый файл для сопоставления алиасов пользователей из внешней системы (источника) с алиасами пользователей в Яндекс 360 (целевой системе). Используется при импорте календарных событий, когда алиасы пользователей в исходной и целевой системах различаются.

Формат файла — значения через `;`:

```
external_email;y360_email
# Это комментарий — строки, начинающиеся с #, пропускаются
old_user;new_user
john.smith@contoso.com;jsmith@domain.ru
petrov;petrov-new
```

| Поле | Описание |
|---|---|
| `external_email` | Алиас или email пользователя из внешней системы (совпадает с алиасом в именах ICS-файлов) |
| `y360_email` | Алиас или email пользователя в Яндекс 360, в чей календарь будут импортированы события |

Правила обработки:

- Первая строка файла — обязательный заголовок `external_email;y360_email`
- Строки, начинающиеся с `#`, являются комментариями и пропускаются
- Значения могут быть указаны как в формате email (`alias@domain.com`), так и в формате алиаса (`alias`) — из email извлекается только часть до `@`
- Все алиасы приводятся к нижнему регистру
- Если файл отсутствует, маппинг не применяется и скрипт продолжает работу в штатном режиме

Механизм работы при импорте:

1. Скрипт считывает файл маппинга и строит словарь соответствий `{внешний_алиас: алиас_я360}`
2. При обработке ICS-файлов для импорта из имени каждого файла извлекается алиас пользователя
3. Если извлечённый алиас найден среди ключей словаря маппинга, он заменяется на соответствующий алиас Яндекс 360
4. Далее импорт выполняется для целевого пользователя Яндекс 360
5. Все операции маппинга логируются в процессе работы и выводятся в итоговом отчёте
...
#### Использование слоёв по умолчанию при импорте

При импорте событий скрипт учитывает информацию о слоях календаря, закодированную в именах ICS-файлов. Слой по умолчанию исходного календаря обозначается фигурными скобками в имени файла (например, `ivanov~{Рабочий}~.ics`). Логика выбора целевого слоя при импорте:

- **Слой по умолчанию исходного календаря не совпадает с именем слоя по умолчанию в целевом календаре.** Если в имени файла слой обёрнут в `{}`, скрипт интерпретирует его как слой по умолчанию исходного календаря и выполняет импорт в слой по умолчанию целевого календаря Яндекс 360, независимо от его имени. Таким образом, совпадение имён слоёв по умолчанию не требуется.
- **Указанный слой отсутствует в целевом календаре.** Если в имени файла указан конкретный слой (без `{}`), но такого слоя нет в целевом календаре пользователя, импорт выполняется в слой по умолчанию целевого календаря. Если слой по умолчанию также не найден, события этого файла будут пропущены.
- **Слой не указан в имени файла.** Если имя файла не содержит информации о слое (например, `ivanov.ics`), импорт выполняется в слой по умолчанию целевого календаря.

#### Использование маппинга пользователей при импорте

Если алиасы пользователей в исходной системе не совпадают с алиасами в Яндекс 360, можно использовать файл маппинга для автоматической подстановки.

**Пример:** ICS-файлы экспортированы из Exchange, где пользователь имел алиас `john.smith`, а в Яндекс 360 его алиас — `jsmith`.

1. Создайте или отредактируйте файл `user_mapping.txt`:
   ```
   external_email;y360_email
   john.smith;jsmith
   petrov@contoso.com;petrov-new@domain.ru
   ```
2. При запуске импорта (пункт меню `3`) скрипт автоматически прочитает файл маппинга.
3. Для файла `john.smith.ics` события будут импортированы в календарь пользователя `jsmith` в Яндекс 360.
4. Все выполненные замены алиасов логируются в процессе работы и выводятся в итоговом отчёте после завершения импорта.

Если файл маппинга отсутствует, скрипт продолжит работу без замен — алиасы из имён файлов будут использованы как есть.
...
Последовательность шагов для создания токена:
    ...
    * заходим на [https://oauth.yandex.ru/client/new/](https://oauth.yandex.ru/client/new/). Аутентифицируемся от имени администратора организации Яндекс 360. Нажимаем на кнопку `Создать приложение` и затем на `Приложение для API или отладки`.
    * <img src="images/create_app_01.jpg" width="500">
    * Указываем произвольное имя приложения.
