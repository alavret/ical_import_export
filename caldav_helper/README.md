...
| `OUTPUT_MAX_MB` | Максимальный размер одного ICS-файла при экспорте в мегабайтах | Нет (по умолчанию `2`) | `2` |
...
1. Перейдите на https://oauth.yandex.ru/client/new/. Аутентифицируйтесь от имени администратора организации Яндекс 360.
2. Заполните поля:
   - **Название сервиса** — произвольное.
   - Включите **Веб-сервисы**.
   - В поле **Redirect URL** укажите `https://oauth.yandex.ru/verification_code`.
...
5. Скопируйте **ClientID** из свойств созданного приложения.
6. Сформируйте ссылку: `https://oauth.yandex.ru/authorize?response_type=token&client_id=<ClientID>` и откройте её в браузере.

> [!WARNING]
> Скопируйте полученный токен и сохраните в надёжном месте.

### Получение ID организации

Зайдите в [консоль администрирования](https://admin.yandex.ru) Яндекс 360. ID организации отображается в левом нижнем углу интерфейса.

### Создание сервисного приложения

Сервисное приложение необходимо для доступа к CalDAV API от имени пользователей организации.

1. Создайте ещё одно OAuth-приложение (аналогично основному), но с разрешением:
   - `calendar:all`

2. Скопируйте **Application ID** и **Application Secret** созданного приложения.
3. Запишите эти значения в параметры `SERVICE_APP_ID` и `SERVICE_APP_SECRET` файла `.env`.

> [!WARNING]
> Для корректной настройки сервисного приложения OAuth-токен основного приложения необходимо выписывать от имени учётной записи владельца организации (в домене `@yandex.ru`).

4. При первом запуске скрипта перейдите в пункт `9. Настройка сервисного приложения.` и выберите `2. Настроить сервисное приложение.`
